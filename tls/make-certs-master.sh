...
pem_admin_dir=/vagrant/artifacts/__CLUSTER_NAME__/tls
pem_admin=$pem_admin_dir/admin.pem
pem_admin_key=$pem_admin_dir/admin-key.pem
pem_admin_csr=$pem_admin_dir/admin-csr.pem

# Generate TLS artifacts
mkdir -p "$cert_dir"

# Create folder for certs
mkdir -p "$pem_admin_dir"
...
cp $pem_ca $pem_ca_key $pem_admin_dir
